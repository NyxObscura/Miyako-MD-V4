...
async function searchShikigami(query) {
  ...
  const url =
    global.proxy +
    "https://shinigami.id/?s=" +
    query +
    "&post_type=wp-manga&op=&author=&artist=&release=&adult="; // Ganti dengan URL yang sesuai
  const response = await fetch(url);
  const html = await response.text();

  const $ = cheerio.load(html);

  const shikigamiArray = $(".c-tabs-item__content")
    .map((index, element) => {
...
async function getDetails(url) {
  try {
    const response = await fetch(global.proxy + url);
    const html = await response.text();
    const $ = cheerio.load(html);
    ...
    manga.info = {
      ...
      alternative: $(
        ...
        .text()
        .trim(),
      authors: $('.post-content_item:contains("Author(s)") .summary-content a')
        .map((index, authorElement) => $(authorElement).text().trim())
        .get(),
      artists: $('.post-content_item:contains("Artist(s)") .summary-content a')
        .map((index, artistElement) => $(artistElement).text().trim())
        .get(),
