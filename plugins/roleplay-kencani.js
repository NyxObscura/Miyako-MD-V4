...
let jarspy = async (m, { conn, text, usedPrefix }) => {
    let user = global.db.data.users[m.sender];  // Mengambil data user dari global.db

    if (user.kepercayaanhusbu > 99) {
        m.reply('*Kamu telah mencapai kepercayaan max*')
        return;
    }
    
    if (new Date() - user.lastkencani < cooldown) {
        m.reply(`Kamu sudah berkencan sebelumnya. Tunggu selama *${((user.lastkencani + cooldown) - new Date()).toTimeString()}* untuk berkencan lagi!`)
    }

    if (!user.husbu) {
        m.reply('Kamu belum mempunyai husbu! Ketik */lamarhusbu* untuk melamar husbu')
        return;
    }
...
⋄ Pagi yang cerah menyapa ${kapital} dan ${user.nama}. Mereka berdua telah merencanakan kencan spesial ini dengan penuh antusiasme. ${kapital} bangun lebih awal untuk bersiap-siap. Dia memakai ${gaun} dan tersenyum senang.`, m);
    }, 1500);
                     
    setTimeout(() => {
        conn.reply(m.chat, `*Jam 07:15 Pagi*

⋄ Sementara itu, ${kapital} sudah bersiap di luar rumah ${user.nama}. Dia membawa ${gift} untuk ${user.nama}. Saat ${user.nama} melihat ${kapital}, senyum mereka bertemu dan mata mereka bersinar.`, m);
    }, 27000);
                     
    setTimeout(() => {
        conn.reply(m.chat, `*Jam 07:30 Pagi*

⋄ Keduanya memutuskan untuk pergi ke ${tempat} untuk ${alesan}. Mereka berjalan berdua, berbicara tentang segala hal, dari hobi mereka hingga impian masa depan. Setiap jalanan dipenuhi dengan bunga-bunga yang berwarna-warni, seperti perasaan mereka satu sama lain.`, m);
    }, 55000);
                     
    setTimeout(() => {
        conn.reply(m.chat, `*Jam 08:00 Pagi - 15:00 Siang*

⋄ Setelah menikmati ${tempat}, ${user.nama} dan ${kapital} pergi ke ${tempat2} untuk ${alesan2}. Mereka saling memandang dengan penuh kasih sayang, merasakan ikatan mereka semakin kuat.`, m);
    }, 75000);
                     
    setTimeout(() => {
        conn.reply(m.chat, `*Jam 15:00 Siang - 22:00 Malam*

⋄ Kencan mereka berlanjut ke ${tempat3}. Hingga malam pun datang begitu cepat, dan mereka merencanakan untuk menonton bintang-bintang bersama.`, m);
    }, 95000);
...
+1 💘 Kepercayaan ${kapital}`.trim(), m);
    }, 115000);
         
    setTimeout(() => {                         
        user.kepercayaanhusbu += 1;  // Update kepercayaan husbu
        user.lastkencani = new Date() * 1;  // Update waktu kencan terakhir
    }, 75000);
};
