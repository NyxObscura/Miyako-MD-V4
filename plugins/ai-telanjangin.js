		...
		react: {
			...
			let anu = await (await fetch(`https://api.itsrose.rest/image/turnMe`, {
				...
				body: JSON.stringify({
					...
					skin: 'chindo',
					image_num: 2,
					prompt: "nude, naked, big breasts, perfect body, nsfw, high quality, without clothes",
				})
			})).json()
			if (!anu.status) return m.reply(anu.styles ? `[ ! ] *example:* *${usedPrefix+command} zombie*\n\n▧「 *available styles* 」\n│✦ ${anu.styles.join('\n│✦ ')}\n└──···` : anu.message)
			m.reply('Photo akan di kirim di private chat')
			for (let x of anu.result.images) {
				await conn.sendFile(m.sender, x, '', `${y < 1 ? `*STYLE: ${teks}*` : `${teks} style (${y+1})`}${prompt ? `\nprompt : ${prompt}` : ''}`, m)
				y++;
			}
		} catch (e) {
			console.log(e)
