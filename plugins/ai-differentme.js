...
const handler = async (m, { conn, usedPrefix, command, args }) => {
  ...
  const style = args[0] || "anime"; // default style is cowboy, usage: avatarme <style>
  ...
  const response = await fetch("https://api.itsrose.rest/image/differentMe", {
    method: "POST",
    headers,
    body,
  });
...
handler.help = ["differentme <reply>"];
handler.tags = ["ai"];
